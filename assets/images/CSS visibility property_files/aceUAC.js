(function() {
 ...
 var ACE3SecureUserMatchingHost = "https://pxl.ace.advertising.com";
 var ACE3UserMatchingHost = "http://cmap.uac.ace.advertising.com";
 var ACE3_ASYNC_FIF_TIMEOUT = 200;
 ...
 var ACE3FVer = "0";
 var ACE3_IFRAME_ID = "ace3adframe";
 var ACE3_RESIZE_DEPTH = 5;
 try {
  var scriptTags = document.getElementsByTagName("script"), i;
  for (i = 0; i < scriptTags.length; i++) {
   var s = scriptTags[i];
   if (s.src && s.src.indexOf("aceUAC.js") > -1) {
    var detectedSecure = s.src.indexOf("secure") > -1;
   }
  }
  var detectedSecure = document.location.protocol === "http:" ? false : true;
 } catch (ignore) {
  var detectedSecure = true;
 }
 var isSecure;
 ...
 var parseSizes = function(sizeStr) {
  var ws = [], hs = [];
  var str, w, h, i, sz = sizeStr.split(",");
 ...
 var getAdDimensionsFromOptions = function(options) {
  var ws = [], hs = [];
  if (options["size"]) {
   return parseSizes(options["size"]);
  }
  if (options["width"] && options["height"]) {
   ws = options["width"].toString().split(",");
   hs = options["height"].toString().split(",");
  } else {
   ws.push("468");
   hs.push("060");
  }
 ...
 var getPossibleAdDimensionsFromOptions = function(options) {
  var dimensions = getAdDimensionsFromOptions(options);
  if (options["possible_size"]) {
   var possibleDimensions = parseSizes(options["possible_size"]);
   var ws = dimensions.widths.concat(possibleDimensions.widths);
   var hs = dimensions.heights.concat(possibleDimensions.heights);
   return {
    widths: ws,
    heights: hs
   };
 ...
 var appendStaticIframe = function(options) {
  ...
  fif.src = (isSecure ? ACE3_UAC_CDN_ORIGIN_SECURE : ACE3_UAC_CDN_ORIGIN) + "/wrapper/aceUAC.htm" + objectToHash(options);
  var adDimensions = getAdDimensionsFromOptions(options);
  fif.id = ACE3_IFRAME_ID;
  fif.setAttribute("scrolling", "no");
  fif.setAttribute("FRAMEBORDER", "0");
  fif.setAttribute("MARGINHEIGHT", "0");
  fif.setAttribute("MARGINWIDTH", "0");
  if (isMultisize) {
   fif.setAttribute("ONLOAD", "ACE3_Resize()");
   fif.width = 0;
   fif.height = 0;
  } else {
   fif.width = adDimensions.widths[0];
   fif.height = adDimensions.heights[0];
  }
 ...
 var getApifw = function() {
  var apifw = "";
  if (window["mraid"]) {
   var version = window["mraid"].getVersion();
 ...
 var ACErtbCheck = function(site, region) {
  if (!document.getElementById("ACE3RTBPingFile") && Math.floor(Math.random() * 100) + 1 <= ACE3RTBglobalThrottle) {
   ...
   rtbPing.src = (isSecure ? ACE3SecureUserMatchingHost : ACE3UserMatchingHost) + "/um.ashx?site=" + site + rcode;
   document.body.appendChild(rtbPing);
  }
 };
 var ACE3_Resize = window["ACE3_Resize"] = function() {
  var adDimensions = window["ACE_SIZES"], dimensions, sizeCommentIdx;
  var iframe = document.getElementById(ACE3_IFRAME_ID);
  var fuzzyMatch = function(w, h) {
   var distance, i, lowestDistance = 100, matchIdx = 0;
   for (i = 0; i < adDimensions.widths.length; i++) {
    distance = Math.abs(adDimensions.widths[i] - w) + Math.abs(adDimensions.heights[i] - h);
   ...
   return {
    width: adDimensions.widths[matchIdx],
    height: adDimensions.heights[matchIdx]
   };
  };
  var resize = function(w, h, d) {
   var i, iframes, prev = self, cur = parent, currentDepth = 0, reachedTop = false;
   while (!reachedTop && currentDepth < d) {
    reachedTop = cur == top;
    iframes = cur.document.getElementsByTagName("iframe");
    for (i = 0; i < iframes.length; i++) {
     if (iframes[i].contentWindow == prev) {
      iframes[i].width = w;
      iframes[i].height = h;
  ...
  var parseSizeComment = function(idx, documentElement) {
   var searchString = documentElement.innerHTML.substring(idx);
   var re = /\"(\d+)\|(\d+)\"/;
   var results = re.exec(searchString);
   if (results.length == 3) {
    var width = Number(results[1]);
    var height = Number(results[2]);
    return {
     width: isNaN(width) ? adDimensions.widths[0] : width,
     height: isNaN(height) ? adDimensions.heights[0] : height
    };
   }
  };
  var findSizeComment = function(documentElement) {
   var sizeCommentVar = "aolsize";
   if (documentElement != null && documentElement.innerHTML != null) {
    return documentElement.innerHTML.toLowerCase().indexOf(sizeCommentVar);
   }
   return -1;
  };
  if (iframe) {
   try {
    if (adDimensions.widths.length == 1) {
     dimensions = {
      width: adDimensions.widths[0],
      height: adDimensions.heights[0]
     };
    } else if ((sizeCommentIdx = findSizeComment(iframe.contentDocument.documentElement)) > -1) {
     dimensions = parseSizeComment(sizeCommentIdx, iframe.contentDocument.documentElement);
    } else {
     dimensions = fuzzyMatch(iframe.contentDocument.body.scrollWidth, iframe.contentDocument.body.scrollHeight);
    }
    iframe.width = dimensions.width;
    iframe.height = dimensions.height;
    if (self != top) {
     resize(dimensions.width, dimensions.height, ACE3_RESIZE_DEPTH);
    }
   } catch (ex) {
    if (iframe.width == 0) {
     iframe.width = adDimensions.widths[0];
     if (iframe.style.width === "0px") {
      iframe.style.width = adDimensions.widths[0] + "px";
     }
    }
    if (iframe.height == 0) {
     iframe.height = adDimensions.heights[0];
     if (iframe.style.height === "0px") {
      iframe.style.height = adDimensions.heights[0] + "px";
     }
    }
   } finally {
    iframe.id = "";
   ...
   try {
    ...
    } else if ((sizeCommentIdx = findSizeComment(self.document.documentElement)) > -1) {
     dimensions = parseSizeComment(sizeCommentIdx, self.document.documentElement);
    } else {
     resize(0, 0, 1);
     dimensions = fuzzyMatch(self.document.body.scrollWidth, self.document.body.scrollHeight);
    }
    resize(dimensions.width, dimensions.height, ACE3_RESIZE_DEPTH);
   } catch (ignore) {}
  }
 ...
 window["ACE3_AdRequest"] = function(obj) {
  ...
  window["ACE_SIZES"] = getPossibleAdDimensionsFromOptions(obj);
  isMultisize = window["ACE_SIZES"].widths.length > 1;
  var site = "";
  if (obj["site"]) {
   site = obj["site"];
  } else {
  ...
  if (obj["secure"] == "0") {
   isSecure = false;
  } else if (obj["secure"] == "1") {
   isSecure = true;
  } else {
   isSecure = detectedSecure;
  }
  var se = -1;
  try {
   se = isSecure ? 1 : -1;
  ...
  if (ct === "J" && ifV > 0 && checkBoolean(obj["async_fif"]) && kvismob === "2" && ACE3AllowExp == 1) {
   if ((ifV === 1 && window.parent !== window.top || ifV === 2) && obj["async_fif_url"]) {
    ...
    var timeout = obj["async_fif_timeout"] ? +obj["async_fif_timeout"] : ACE3_ASYNC_FIF_TIMEOUT;
  ...
  var adv = ".advertising.com", alt = "Click to learn more...", gl = "", bnum, szs = "", parm = "";
  if (obj["ud"]) {
  ...
  var w = adDimensions.widths[0];
  var h = adDimensions.heights[0];
  var sizes = "";
  for (i = 0; i < adDimensions.widths.length; i++) {
   sizes += adDimensions.widths[i] + adDimensions.heights[i] + ",";
  }
  var sz = obj["size"] || sizes;
  ...
  if (window.devicePixelRatio) {
   sr1 += "/sd=" + window.devicePixelRatio;
  }
  var pNo = "1";
  var pf = obj["popfreq"];
  if (pf) {
  ...
  if (ct == "IMAGE" || ct == "I") {
   ...
   if (ct == "IFRAME" || ct == "IF") {
    ...
    dr = ACE3CkLen(s1, dr);
    if (isMultisize) {
     dw = "<IFRAME ID='" + ACE3_IFRAME_ID + "' SRC='" + s1 + dr + "' WIDTH=0 HEIGHT=0 ONLOAD=ACE3_Resize(); SCROLLING=NO FRAMEBORDER=0 MARGINHEIGHT=0 MARGINWIDTH=0></IFRAME>";
    } else {
    ...
    if (isMultisize) {
     dw += "<SCRIPT>";
     dw += "document.onreadystatechange = function () {if (document.readyState == 'complete') ACE3_Resize();};";
     dw += "</SCRIPT>";
    }
   }
  }
